...
const Initialization = (status: boolean) =>
  ({ type: "auth/INITIALIZATION", status } as const);
const captchaUrl = (url: string | null) =>
  ({ type: "auth/CAPTCHA", payload: { url } } as const);

// thunks
export const appInitializationAsync = (): ThunkAction<
  void,
  AppRootState,
  unknown,
  AuthActionsType
